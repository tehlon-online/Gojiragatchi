...
export default async function handler(req, res) {
  ...
  const sysPath = path.join(process.cwd(), 'config', 'system_instructions.txt');
  const rulesPath = path.join(process.cwd(), 'config', 'game_rules.txt');
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'gpt-3.5-turbo', // Changed from gpt-4 to gpt-3.5-turbo for efficiency
      messages
    })
  });
